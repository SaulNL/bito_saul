...
export class InfoNegocioPage implements OnInit {
  ...
  public buscarNegocio(id) {
    if (
      this.negocioTO.id_negocio === null ||
      this.negocioTO.id_negocio === undefined
    ) {
      //this.negocioTO = new NegocioModel();
      //this.negocioTO.tags = "";
            ...
            if (width === 400 && height === 400) {
              ...
              if (file.size < 3145728) {
                ...
                utl.getBase64(file).then((data) => {
                  const archivo = new ArchivoComunModel();
                  if (file_name != null) {
                    archivo.nombre_archivo = this._utils_cls.convertir_nombre(
                      file_name
                    );
  ...
  confirmarUrlNegocio(evento, entrada = 1) {
    ...
    this.controladorTiempo = setTimeout(() => {
      ...
      this.negocioServico.verificarUrlNegocio(tem).subscribe((repuesta) => {
  ...
  agregarHorario() {
    if (
      ...
      this.nuevoHorario.id_horario === undefined
    ) {
      this.nuevoHorario.hora_inicio = moment
        .parseZone(this.horarioini)
        .format("HH:mm");
      this.nuevoHorario.hora_fin = moment
        .parseZone(this.horariofin)
        .format("HH:mm");
  ...
  datos() {
    ...
    if (
      this.negocioGuardar.organizaciones !== undefined &&
      this.negocioGuardar.organizaciones.length > 0
    ) {
  ...
  editarHorario(horario, i) {
    ...
    this.horarioini = moment
      .parseZone(objFecha)
      .format("YYYY-MM-DDT" + horario.hora_inicio + ":ssZ");
    this.horariofin = moment
      .parseZone(objFecha)
      .format("YYYY-MM-DDT" + horario.hora_fin + ":ssZ");
